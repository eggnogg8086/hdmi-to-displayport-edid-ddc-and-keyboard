...
if (Test-Path $cli) {
    $restore = Start-Process -FilePath $cli -ArgumentList '--on' -Wait -PassThru -WindowStyle Hidden
    if ($restore.ExitCode -ne 0) {
        # Do not remove the only automatic recovery path while the panel could
        # still be black. Put the task back and make the user retry while the
        # display output/DDC link is awake.
        Start-ScheduledTask -TaskName $taskName -ErrorAction SilentlyContinue
        throw "Brightness restore failed (exit $($restore.ExitCode)). Uninstall was aborted for safety. Wake the screen, use Emergency Recover if needed, then try uninstall again."
    }
    Start-Sleep -Milliseconds 250
} else {
    Start-ScheduledTask -TaskName $taskName -ErrorAction SilentlyContinue
    throw 'Recovery CLI is missing, so safe brightness restore cannot be verified. Uninstall was aborted. Reinstall/repair the package first, then uninstall.'
}

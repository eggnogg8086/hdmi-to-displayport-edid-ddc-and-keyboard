...
if ($existingTask) {
    ...
    Write-Host 'Removed previous scheduled task; it will be recreated only after safety checks pass.'
}
...
# User-editable configuration lives in ProgramData, not Program Files. Preserve
# it across upgrades so local timing/brightness preferences are never reset.
$settingsPath = 'C:\ProgramData\ESP32BrightnessBridge\settings.ini'
$defaultSettings = Join-Path $root 'settings.ini'
if (-not (Test-Path $settingsPath)) {
    if (-not (Test-Path $defaultSettings)) {
        throw "Default settings.ini is missing from the installation package."
    }
    Copy-Item $defaultSettings $settingsPath -Force
    Write-Host 'Created user settings: ' $settingsPath
} else {
    Write-Host 'Preserving existing user settings: ' $settingsPath
}

# Preserve an existing known-good restore level across upgrades. An update can be
# launched while the old agent is temporarily dimmed, so relearning the panel
# unconditionally could turn a transient DIM value into the user's permanent one.
$existingBrightness = $null
try {
    $existingBrightness = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\ESP32BrightnessBridge' -Name CurrentBrightness -ErrorAction Stop).CurrentBrightness
} catch {}
$hasExistingBrightness = ($null -ne $existingBrightness -and $existingBrightness -ge 1 -and $existingBrightness -le 100)

New-Item -Path 'HKLM:\SOFTWARE\ESP32BrightnessBridge' -Force | Out-Null
# Do NOT invent a 50% restore value here. v10.3 did that before it had read the
# monitor, which could later overwrite the user's real brightness. The safety
# probe below first adopts the current nonzero DDC value. A 50% fallback is
# created only if the panel is already at zero and no previous restore value exists.
...
try {
    ...
    $regRule = [System.Security.AccessControl.RegistryAccessRule]::new(
        $targetUser,
        ([System.Security.AccessControl.RegistryRights]::ReadKey -bor [System.Security.AccessControl.RegistryRights]::SetValue),
...
if ($svc) {
    ...
    Write-Host 'Removed obsolete ESP32DisplayPowerBridge service.'
}

# SAFETY: prove that AUOD0A2 DDC can be opened BEFORE writing any brightness.
# Then adopt the panel's current nonzero brightness as the initial restore value.
# This preserves whatever level the user had before installing the bridge.
...
if ($hasExistingBrightness) {
    Write-Host "Preserving existing saved brightness: $existingBrightness%"

    # Preserve any TemporaryDimActive/TemporaryDimValue marker written by the
    # previous agent. v10.8.1.1 supports user-configurable DIM percentages, so the
    # installer must not guess a temporary value during an upgrade.
} else {
    $sync = Start-Process -FilePath $dstCli -ArgumentList '--sync' -Wait -PassThru -WindowStyle Hidden
    if ($sync.ExitCode -ne 0) {
        # A zero VCP value most likely means a panel left dark by an earlier crash.
        # On a brand-new install there is no known restore level, so use a
        # conservative 50% recovery value only for this exceptional case.
        ...
        $restore = Start-Process -FilePath $dstCli -ArgumentList '--on' -Wait -PassThru -WindowStyle Hidden
        if ($restore.ExitCode -ne 0) {
            throw "Could not establish a usable nonzero brightness on first install (exit $($restore.ExitCode)). Automatic control was NOT enabled."
        }
    }
}
...
if (-not $agent) {
    ...
    $restore = Start-Process -FilePath $dstCli -ArgumentList '--on' -Wait -PassThru -WindowStyle Hidden
    if ($restore.ExitCode -ne 0) {
        throw "The hidden agent did not remain running, and brightness recovery also failed (exit $($restore.ExitCode)). The task was removed. Use Emergency Recover after waking the display output."
    }
...
try {
    ...
    New-ItemProperty -Path $uninstallKey -Name DisplayVersion -PropertyType String -Value '10.8.1' -Force | Out-Null
    ...
    New-AppShortcut -Path (Join-Path $startMenuDir 'Edit Settings.lnk') -Target (Join-Path $installDir 'Edit Settings.cmd') -WorkingDirectory $installDir
...
Write-Host ''
Write-Host 'ESP32 Brightness Bridge 10.8.1 installed and started.'
Write-Host 'Installed Apps entry: ESP32 Brightness Bridge'
Write-Host 'Agent:' $dstAgent
Write-Host 'Settings: C:\ProgramData\ESP32BrightnessBridge\settings.ini'
